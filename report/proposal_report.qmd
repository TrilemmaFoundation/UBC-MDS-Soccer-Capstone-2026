...
**Match-level context and quality.** Competition coverage is uneven: La Liga contributes 868 matches across full seasons, while the Champions League includes scattered single-match history. Match outcomes show a familiar home skew (≈45% home wins, 29% away, 26% draws). Incremental dbt materialization is sized for the full events volume (~12.2M rows) without loading the entire file into memory each run.
