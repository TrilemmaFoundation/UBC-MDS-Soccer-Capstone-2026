...
::: {.notes}
Trilemma had rich StatsBomb data but no scalable way to answer these questions without SQL — this table is our map for the talk. Next: how the data actually moves, starting with dbt.
:::
...
::: {.notes}
First result — clustering. Crop the Looker scatter so archetype names and player counts are large and legible. Don't show the full dashboard chrome — just the insight.
:::
...
::: {.notes}
Second result — consistency. Crop the quadrant scatter so axis labels, quadrant names, and a few player names are clearly visible. Rabin will go deeper into this later.
:::
...
::: {.notes}
Third result — the chatbot answering a real question. Show both the prompt and the response large enough to read. Li will now walk through RQ1 and RQ2 — clustering and the Player Archetypes dashboard.
:::
...
::: {.notes}
For RQ2, we cluster outfield players by playing style. Per-season stats from dbt, 270-minute minimum, goalkeepers excluded, 13 features through PCA and KMeans. Output lands in BigQuery and feeds the dashboard and chatbot.
:::
...
::: {.notes}
The scatter plot in Looker shows PCA space with color-coded clusters. Analysts can instantly find players with a similar tactical profile.
:::
...
::: {.notes}
Looker Page 1 — overview. Crop so the headline numbers and archetype counts are clearly readable, not the full browser window.
:::
...
::: {.notes}
Looker Page 2 — explorer and top-player tables. We showed how we cluster players into tactical archetypes. Our second objective is consistency scoring — Rabin covers RQ3 next.
:::
...
- **ML** — `cluster.py` + `consistency.py`
- **Serving** — Django app, Looker embed, LLM chatbot
